...
export const ResultsTable: React.FC<ResultsTableProps> = ({ metrics, teams, seniors, newHires }) => {
  ...
  const filteredMetrics = useMemo(() => {
    ...
    // Remove metadata rows (Salesforce report headers/footers) and total/subtotal rows
    filtered = filtered.filter(m => {
      const name = m.agentName;
      const nameLower = name.toLowerCase();

      // Filter out Salesforce metadata rows
      if (name.length > 80) return false; // Real names are short
      if (/^as of \d{4}/i.test(name)) return false;
      if (/^(channel|date field|field |filtered by|show:|trip channel|new value|passthrough date vs|passthrough to sales date|last gtt action by|gtt owner|you've reached)/i.test(name)) return false;
      if (/equals /i.test(name)) return false;
      if (/generated by .* sorted by/i.test(name)) return false;
      if (/ [↑↓]$/.test(name)) return false;

      // Filter out total/subtotal rows with zero values
            ...
            {sortedMetrics.map((m, idx) => {
              ...
              return (
                <tr key={m.agentName} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] max-w-[200px] truncate ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    <div className="flex items-center gap-2">
                      {m.agentName}
